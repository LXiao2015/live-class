...
/// 音视频通话的通话能力权限位。
#define AUTH_BITS_DEFUALT 0xFFFFFFFFFFFFFFFFUL ///< 所有权限。
...
class AV_EXPORT AVRoom {
 public:
  ...
  struct Info {
    ...
    RelationType
    relation_type; ///< 关系类型，多人房间专用。业务侧开发人员在接入SDK时，固定将该参数设置为6即可。
    ...
    std::string
    av_control_role; ///角色名，多人房间专用。该角色名就是web端音视频参数配置工具所设置的角色名。
  };
  ...
  */
  virtual bool GetStatParam(RoomStatParam* param) = 0;
