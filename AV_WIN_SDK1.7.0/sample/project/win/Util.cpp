...
{
	std::wstring str = w.GetString();//wstring是宽char，Unicode编码，一般情况下一个字符占两个字节大小
...
void LoadBmpData(std::string strFileName, uint8** ppData, uint32 & nWidth, uint32 & nHeight)  //加水印
{
...
void _CopyBits2Tex_None_0(uint8* pDst, uint8* pSrc, UINT uLen, const SIZE & size)//旋转
{
...
bool _ResizeWithMendBlack(uint8* pDst, uint8* pSrc, UINT uDstLen, UINT uSrcLen, const SIZE & dstSize, const SIZE & srcSize, UINT bpp)  //补黑边

{
  if(!pDst || !pSrc)
    return false;

  if(uDstLen == 0 || uDstLen != dstSize.cx * dstSize.cy * bpp)  //bpp是像素深度，指存储每个像素所用的位数，用来度量图像的分辨率
    return false;
