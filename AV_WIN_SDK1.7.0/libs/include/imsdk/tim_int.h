...
struct TIMStreamRsp
{
	TIMStreamRsp(): channel_id(0){}  //构造函数初始化列表以一个冒号开始，接着是以逗号分隔的数据成员列表，每个数据成员后面跟一个放在括号中的初始化式
...
{
	TIMStreamParam()
		:encode(HLS), sdk_type(SDKType_Normal), push_data_type(0), tape_flag(0){}
...
public:
	...
	void RequestSharpSvr(uint64_t touin, const std::string &req, TIMCallBack *cb, uint64_t timeout = 5000);
	void ResponseSharpSvr(uint64_t touin, const std::string &req, TIMCallBack *cb, uint64_t timeout = 5000);
